...
const DailyStatModal = () => {
  ...
  const handleSubmit = async e => {
    ...
    console.log('from.match(dateRegex)', from, dateRegex.test(from));

    if (dateRegex.test(from) === false || dateRegex.test(to) === false) {
      console.log(true);
      document.getElementById('reply').innerText = 'Неправильний формат дат. Використайте yyyy-mm-dd';
      return;
    }
    ...
    if (diffDays > 30) {
      console.log('here');
      document.getElementById('reply').innerText = 'Обраний період перевищує 30 днів. Будь ласка, скористайтеся віджетом місячної статистики';
      return;
    }

    await api
      .get_daily_stat(from, to, localStorage.getItem('card_id'))
      .then(apiResponse => {
        console.log('apiResponse', apiResponse.data);
        var spends = apiResponse.data.spendings;
        if (!spends) {
          spends = 0;
        }
        console.log('spends', spends);
        document.getElementById('reply').innerText = `Витрачена сума за обраний період становить ${spends} гривень.`;
        console.log('Displayed spends');
      })
      .catch(error => {
        console.log('error', error);
        document.getElementById('reply').innerText = "На жаль, сталася помилка. Спробуйте пізніше";
      })
  }

  return (
    <div className="modal fade" id="dailyStatModal" tabIndex="-1" role="dialog" aria-labelledby="dailyStatModal" aria-hidden="true">
      <div className="modal-dialog modal-dialog-centered" role="document">
        <div className="modal-content bank-modal">
          ...
          <div className="bank-modal-content">
            <form noValidate> 
              <p>Введіть проміжок дат, за яким ми підсумуємо кількість витрат (не більше 30 днів)</p>
              <p>Щоб агрегувати більший проміжок часу, скористайтеся місячною статистикою</p>
              <TextField
                fullWidth
                label="Дата ВІД у форматі рік-місяць-день"
                name="from"
                variant="outlined"
              ...
              <TextField
                fullWidth
                label="Дата ДО у форматі рік-місяць-день"
                name="to"
                variant="outlined"
                className={styles.customInput + " " + classes.root}
                value={formState.to}
                onChange={e => handleChange(e)}
              />
              <button className="bank-btn" onClick={handleSubmit} >
                <i className="fa fa-calculator" aria-hidden="true"></i>
                <span>Порахувати</span>
              </button>
              <p className='mt-20' id='reply'></p>
            </form>
          </div>
        </div>
      </div>
