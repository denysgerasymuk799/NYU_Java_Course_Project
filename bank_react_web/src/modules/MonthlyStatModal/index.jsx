...
const MonthlyStatModal = () => {
  ...
  const handleSubmit = async e => {
    ...
    if (dateRegex.test(from) === false || dateRegex.test(to) === false) {
      console.log(true);
      document.getElementById('reply-mo').innerText = 'Неправильний формат дат. Використайте yyyy-mm';
      return;
    }

    await api
      .get_monthly_stat(`${from}-01`, `${to}-01`, localStorage.getItem('card_id'))
      .then(apiResponse => {
        ...
        document.getElementById('reply-mo').innerText = `Витрачена сума за обраний період становить ${spends} гривень.`;
      })
      .catch(error => {
        console.log('error', error);
        document.getElementById('reply-mo').innerText = "На жаль, сталася помилка. Спробуйте пізніше";
      })
  }

  return (
    <div className="modal fade" id="monthlyStatModal" tabIndex="-1" role="dialog" aria-labelledby="monthlyStatModal" aria-hidden="true">
      <div className="modal-dialog modal-dialog-centered" role="document">
        <div className="modal-content bank-modal">
          ...
          <div className="bank-modal-content">
            <form noValidate> 
              <p>Введіть проміжок дат, за яким ми підсумуємо кількість витрат</p>
              <p>Щоб агрегувати менший проміжок часу, скористайтеся денною статистикою</p>
              <TextField
                fullWidth
                label="Дата ВІД у форматі рік-місяць"
                name="from"
                variant="outlined"
              ...
              <TextField
                fullWidth
                label="Дата ДО у форматі рік-місяць"
                name="to"
                variant="outlined"
                className={styles.customInput + " " + classes.root}
                value={formState.to}
              ...
              <p className='mt-20' id='reply-mo'></p>
            </form>
          </div>
        </div>
      </div>
...
export default MonthlyStatModal;
